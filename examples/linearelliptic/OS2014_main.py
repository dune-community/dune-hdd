...
if __name__ == '__main__':
    ...
    if not os.path.isfile(config_filename):
        logger.info('writing default configuration to {}...'.format(config_filename))
        with open(config_filename, 'w') as f:
            print('''# OS2014 config file
[dune]
partitioning = [1 1 1]
num_refinements = 0
info_log_levels = 1
...
''', file=f)
    ...
    try:
        ...
        assert config.has_section('dune')
        assert config.has_section('pymor')
    except:
        raise ConfigError('CONFIGFILE has to be the name of an existing file that contains a [dune] and a [pymor] section')
    ...
    example = dune_module.OS2014Spe10Model1Example(config.get('dune', 'partitioning'),
                                                   config.getint('dune', 'num_refinements'),
                                                   config.getint('dune', 'info_log_levels'),
                                                   config.getint('dune', 'debug_log_levels'),
                                                   config.getboolean('dune', 'enable_warnings'),
                                                   config.getboolean('dune', 'enable_colors'),
                                                   config.get('dune', 'info_color'),
                                                   config.get('dune', 'debug_color'),
                                                   config.get('dune', 'warn_color'))
    ...
    t = type(example.discretization())
    wrapped_class = wrap_multiscale_discretization(t, wrapper)
    wrapper.add_class(t, wrapped_class)
    multiscale_discretization = wrapper[example.discretization()]
    multiscale_discretization = multiscale_discretization.with_(
        parameter_space=CubicParameterSpace(multiscale_discretization.parameter_type, 0.1, 1.0))
    reduction_report, data = perform_lrbms(config, multiscale_discretization, training_samples)
    logger.info(reduction_report)
    ...
    if test_set_sampling_strategy == 'training':
        ...
        num_test_samples = config.getint('pymor', 'num_test_samples')
        test_samples = list(multiscale_discretization.parameter_space.sample_randomly(num_test_samples))
    else:
        raise ConfigError('unknown \'test_set\' sampling strategy given: \'{}\''.format(test_set_sampling_strategy))
    test_report = test_quality(config, test_samples, multiscale_discretization, data, test_set_sampling_strategy)
    logger.info(test_report)
