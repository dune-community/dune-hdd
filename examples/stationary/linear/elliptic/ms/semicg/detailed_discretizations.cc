...
void writeDescriptionFile(std::string filename)
{
  ...
  file.open(filename);
  file << "[" << id() << "]" << std::endl;
  file << "model = model.stationary.linear.elliptic.parametric.separable.twophase" << std::endl;
...
//   file << "        model.stationary.linear.elliptic.thermalblock" << std::endl;
  ...
  file << "upperRight = [5.0; 1.0]" << std::endl;
  file << "numElements = [200; 40]" << std::endl;
  file << "boundaryId = 7 # a cube from the factory gets the boundary ids 1 to 4 ind 2d and 1 to 6 in 3d (hopefully)" << std::endl;
  file << "partitions = [10; 3]" << std::endl;
  file << "oversamplingLayers = 5" << std::endl;
...
//   file << "neumann.order      = 0"  << std::endl;
//   file << "neumann.name       = trivial neumann"  << std::endl;
  file << "[model.stationary.linear.elliptic.parametric.separable.twophase]" << std::endl;
  file << "totalMobility.type = function.separable.default" << std::endl;
  file << "totalMobility.name = total mobility (two drops)" << std::endl;
  file << "totalMobility.order = 2" << std::endl;
  file << "totalMobility.component.0 = -1.0*exp(-1.0*(((x[0]-0.75)*(x[0]-0.75))/(2*0.1*0.1)))" << std::endl;
  file << "totalMobility.component.1 = -1.0*exp(-1.0*(((x[0]-4.25)*(x[0]-4.25))/(2*0.1*0.1)))" << std::endl;
  file << "totalMobility.component.2 = 1.0" << std::endl;
  file << "totalMobility.coefficient.0 = mu[0]" << std::endl;
  file << "totalMobility.coefficient.1 = mu[1]" << std::endl;
  file << "totalMobility.paramSize = 2" << std::endl;
  file << "totalMobility.paramMin = [0.0; 0.0]" << std::endl;
  file << "totalMobility.paramMax = [0.99; 0.99]" << std::endl;
  file << "totalMobility.paramExplanation = [drop_at_0.75; drop_at_4.25]" << std::endl;
  file << "permeability.type = function.spe10.model1" << std::endl;
  file << "permeability.filename = /home/felix/Desktop/spe10/dataset01/perm_case1.dat" << std::endl;
  file << "permeability.name = permeability (spe10.model1)" << std::endl;
  file << "permeability.order = 0" << std::endl;
  file << "permeability.lowerLeft = [0.0; 0.0]" << std::endl;
  file << "permeability.upperRight = [5.0; 1.0]" << std::endl;
  file << "permeability.minValue = 0.1" << std::endl;
  file << "permeability.maxValue = 10" << std::endl;
  file << "force.type       = function.expression" << std::endl;
  file << "force.order      = 0" << std::endl;
  file << "force.variable   = x" << std::endl;
  file << "force.expression = 10.0*exp(-1.0*((((x[0]-0.75)*(x[0]-0.75))+((x[1]-0.5)*(x[1]-0.5)))/(2*0.25*0.25)))+10.0*exp(-1.0*((((x[0]-4.25)*(x[0]-4.25))+((x[1]-0.5)*(x[1]-0.5)))/(2*0.25*0.25)))-10.0*exp(-1.0*((((x[0]-2.5)*(x[0]-2.5))+((x[1]-0.5)*(x[1]-0.5)))/(2*0.25*0.25)))" << std::endl;
  file << "force.name       = force (two wells, one producer)" << std::endl;
  file << "dirichlet.order      = 0" << std::endl;
  file << "dirichlet.variable   = x" << std::endl;
  file << "dirichlet.expression = [0.0; 0.0; 0.0]" << std::endl;
  file << "dirichlet.name       = dirichlet (0)" << std::endl;
  file << "neumann.order      = 0" << std::endl;
  file << "neumann.variable   = x" << std::endl;
  file << "neumann.expression = [0.; 0.0; 0.0]" << std::endl;
  file << "neumann.name       = neumann (0)" << std::endl;
  file << "[logging]" << std::endl;
  file << "info  = true" << std::endl;
  file << "debug = true" << std::endl;
  file << "file  = false" << std::endl;
  file << "[parameter]" << std::endl;
  file << "fixed = [0.0; 0.0]" << std::endl;
  file << "train.0 = [0.0; 0.0]" << std::endl;
  file << "train.1 = [0.99; 0.0]" << std::endl;
//   file << "train.2 = [0.0; 0.99]" << std::endl;
//   file << "train.3 = [0.99; 0.99]" << std::endl;
  file << "test.0  = [0.0; 0.99]" << std::endl;
  ...
  file << "gramSchmidt = true" << std::endl;
  file << "gramSchmidtTolerance = 1e-8" << std::endl;
  file << "[localErrors]" << std::endl;
  file << "scalarproduct = fixed" << std::endl;
  file << "                current" << std::endl;
  file << "[markSubdomains]" << std::endl;
  file << "strategy = factorTimesMean" << std::endl;
  file << "           all" << std::endl;
  file << "markingFactor = 1.0" << std::endl;
  file << "[localCorrections]" << std::endl;
  file << "cheat = false" << std::endl;
  file << "useCorrection = true" << std::endl;
  file.close();
...
//  const bool debugLogging = description.get< bool >("logging.debug", false);
  ...
  info << "assembling detailed operator ("
       << model->diffusion()->numComponents() << " components) and functional... "
       << std::flush;
  timer.reset();
                                                  ...
                                                  std::vector< Dune::shared_ptr< ReducedBasisType > >& localReducedBases)
{
  ...
  const RangeFieldType gramSchmidtTolerance = description.get< RangeFieldType >("enrichment.gramSchmidtTolerance", 1e-10);
...
VectorType computeLocalErrors(const SolverType& fixedGlobalSolver,
                              ...
                              const DetailedOperatorType& detailedOperator,
                              const DetailedFunctionalType& detailedFunctional,
                              const std::vector< Dune::shared_ptr< ScalarProductType > >& localFixedScalarProducts,
                              const std::vector< Dune::shared_ptr< ReducedBasisType > >& localReducedBases,
                              const MapperType& mapper,
                              const VectorType& reducedSolution,
                              const ParamType& mu,
                              ...
                              Dune::shared_ptr< VectorType >& tmpGlobalVector)
{
  ...
  const std::string scalarProductType = description.get< std::string >("localErrors.scalarproduct", "fixed");

  std::vector< Dune::shared_ptr< ScalarProductType > > localScalarProducts;

  if (scalarProductType == "fixed") {
    localScalarProducts = localFixedScalarProducts;
  } else if (scalarProductType == "current") {
    SolverType localCurrentScalarProductSolver(fixedGlobalSolver.model()->fix(mu),
                                               fixedGlobalSolver.msGrid(),
                                               fixedGlobalSolver.boundaryInfo(),
                                               fixedGlobalSolver.penaltyFactor());
    localCurrentScalarProductSolver.init("", devnull);
    localScalarProducts = localCurrentScalarProductSolver.systemMatrices(ParamType());
  } else
    DUNE_THROW(Dune::InvalidStateException,
               "\n" << Dune::Stuff::Common::colorStringRed("ERROR:")
               << " wrong 'localErrors.scalarproduct' given (has to be one of 'fixed', 'current', is'"
               << scalarProductType << "')!");
  ...
  // visualize difference (if wished)
  if (visualize) {
  ...
  if (visualize) {
    for (size_t ss = 0; ss < tmpLocalVectorsA.size(); ++ss)
      for (int ii = 0; ii < tmpLocalVectorsA[ss]->size(); ++ii)
        tmpLocalVectorsA[ss]->set(ii, localErrors.get(ss));
    fixedGlobalSolver.visualize(tmpLocalVectorsA,
                                "localRelativeErrors.param_" + Dune::Stuff::Common::Parameter::reportWOwhitespace(mu),
                                "local relative errors to parameter " + Dune::Stuff::Common::Parameter::report(mu),
                                "",
                                devnull);
  } // visualize error (if wished)
                             ...
                             Dune::shared_ptr< VectorType >& tmpGlobalVector)
{
  ...
  const RangeFieldType linearSolverPrecision = discretizationDescription.get< RangeFieldType >("linearsolver.precision",
                                                                                               1e-12);
  const bool cheat = description.get< bool >("localCorrections.cheat", false);
  const bool useCorrection = description.get< bool >("localCorrections.useCorrection", true);
  ...
  if (cheat) {
    ...
    if (useCorrection) {
      // correct rhs
      tmpGlobalVector = fixedSolver.globalizeVectors(tmpLocalVectorsA);
    ...
    // solve
    fixedSolver.solve(tmpLocalVectorsB,       // <- solution
                      linearSolverType,
                      linearSolverMaxIter,
                      linearSolverPrecision,
                      "",
                      devnull);
    computeGlobalSnapshot(description, globalSolver, detailedOperator, detailedFunctional, mu, tmpLocalVectorsA, tmpGlobalVector);
    ...
    fixedSolver.visualize(tmpLocalVectorsA,
                          "biasGlobalSnapshot_parameter_" + Dune::Stuff::Common::Parameter::reportWOwhitespace(mu),
                          "bias in global snapshot to parameter " + Dune::Stuff::Common::Parameter::reportWOwhitespace(mu),
                          "", devnull);
    SolverType localCurrentScalarProductSolver(globalSolver.model()->fix(mu),
                                               globalSolver.msGrid(),
                                               globalSolver.boundaryInfo(),
                                               globalSolver.penaltyFactor());
    localCurrentScalarProductSolver.init("", devnull);
    const auto localScalarProducts = localCurrentScalarProductSolver.systemMatrices(ParamType());
    VectorType localBiases(localScalarProducts.size());
    for (size_t ss = 0; ss < localScalarProducts.size(); ++ss)
      localBiases.set(ss,
                      std::sqrt(tmpLocalVectorsA[ss]->backend().transpose()
                                * localScalarProducts[ss]->backend()
                                * tmpLocalVectorsA[ss]->backend())
                      / std::sqrt(tmpLocalVectorsB[ss]->backend().transpose()
                                  * localScalarProducts[ss]->backend()
                                  * tmpLocalVectorsB[ss]->backend()));
    debug << "done (took " << timer.elapsed() << "sec)" << std::endl;
    debug << "relative mean local bias in global snapshot is " << localBiases.backend().mean() << std::endl;
  } else {
    ...
    for (int ii = 0; ii < markedSubdomains.size(); ++ii) {
      ...
      if (visualize) {
        ...
        localSolver.visualizeAnsatzVector(localSolution,
                                          ...
                                          "",
                                          devnull);
//      auto localOversampledDifferenceFunction = localOversampledSolver.createAnsatzFunction();
//      Dune::Stuff::HeterogenousProjection<>::project(*globalCorrectionFunction,
//                                                     *localOversampledDifferenceFunction);
//      localOversampledDifferenceFunction->vector()->backend() -= localOversampledSolution->backend();
//      localOversampledSolver.visualizeAnsatzVector(localOversampledDifferenceFunction->vector(),
...
//                            "", devnull);
//      auto localDifferenceFunction = localSolver.createAnsatzFunction();
//      Dune::Stuff::HeterogenousProjection<>::project(*globalCorrectionFunction,
//                                                     *localDifferenceFunction);
//      localDifferenceFunction->vector()->backend() -= localSolution->backend();
//      localSolver.visualizeAnsatzVector(localDifferenceFunction->vector(),
...
//              markedSubdomains.set(ii, ii);
            ...
            computeLocalCorrections(*fixedGlobalSolver,
                                    ...
                                    *mapper,
                                    ...
                                    tmpLocalVectorsA,
                                    tmpLocalVectorsB,
                                    tmpGlobalVectorA); // <- the local corrections will be in here
