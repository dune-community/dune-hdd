...
public:
  ...
  typedef typename DiscretizationType::RangeFieldType RangeFieldType;
...
protected:
  static void assert_parameters(const DiscretizationType& disc, const ParametersType& parameters)
  {
    if (parameters.find("fixed") == parameters.end())
      DUNE_THROW(Stuff::Exceptions::wrong_input_given, "Missing 'fixed' parameter!");
    if (parameters.find("minimizing") == parameters.end())
      DUNE_THROW(Stuff::Exceptions::wrong_input_given, "Missing 'minimizing' parameter!");
    if (parameters.find("current") == parameters.end())
      DUNE_THROW(Stuff::Exceptions::wrong_input_given, "Missing 'current' parameter!");
    for (const auto& parameter : parameters)
      if (parameter.second.type() != disc.parameter_type())
        DUNE_THROW(Pymor::Exceptions::wrong_parameter_type,
                   "Is " << parameter.second.type() << ", should be " << disc.parameter_type() << "!");
  } // ... assert_parameters(...)

  typedef typename DiscretizationType::MsGridType      MsGridType;
  typedef typename DiscretizationType::AnsatzSpaceType AnsatzSpaceType;
  typedef typename DiscretizationType::ProblemType     ProblemType;
  typedef typename DiscretizationType::DomainFieldType DomainFieldType;
  typedef typename DiscretizationType::EntityType      EntityType;
  static const unsigned int                            dimDomain = DiscretizationType::dimDomain;

  static RangeFieldType compute_nonconformity_estimator(const DiscretizationType& disc,
                                                        const VectorType& vector,
                                                        const ParametersType& parameters)
  {
    using namespace GDT;

    const auto mu_fixed = parameters["fixed"];

    typedef typename MsGridType::GlobalGridViewType GlobalGridViewType;
    const auto grid_view = disc.ms_grid_->globalGridView();
    ...
    const auto diffusion_factor_fixed = disc.problem().diffusion_factor()->with_mu(disc.map_parameter(mu_fixed,
                                                                                                      "diffusion_factor"));
    typedef typename ProblemType::DiffusionTensorType::NonparametricType DiffusionTensorType;
    const auto diffusion_tensor_fixed = disc.problem().diffusion_tensor()->with_mu(disc.map_parameter(mu_fixed,
                                                                                                      "diffusion_tensor"));
    const LocalOperator::Codim0Integral< LocalEvaluation::Elliptic< DiffusionFactorType, DiffusionTensorType > >
      local_eta_nc_product(over_integrate, *diffusion_factor_fixed, *diffusion_tensor_fixed);
    const auto eta_nc_difference = discrete_solution - oswald_interpolation;

    std::vector< DynamicMatrix< RangeFieldType > > tmp_matrices(local_eta_nc_product.numTmpObjectsRequired(),
                                                                DynamicMatrix< RangeFieldType >(1, 1, 0.0));
    DynamicMatrix< RangeFieldType > local_result_matrix(1, 1, 0.0);

    // walk the subdomains
    double eta_nc_squared = 0.0;
    for (size_t subdomain = 0; subdomain < disc.ms_grid_->size(); ++subdomain) {
      const auto local_grid_part = disc.ms_grid_->localGridPart(subdomain);
    ...
    return std::sqrt(eta_nc_squared);
  } // ... compute_nonconformity_estimator(...)

  static RangeFieldType compute_residual_estimator(const DiscretizationType& disc,
                                                   const VectorType& vector,
                                                   const ParametersType& parameters)
  {
    using namespace GDT;

    const auto mu = parameters["current"];
    const auto mu_minimizing = parameters["minimizing"];
    ...
    assert(!disc.problem().force()->parametric());
    const auto force = disc.problem().force()->with_mu(disc.map_parameter(mu, "force"));
    projection_operator.apply(*force, p0_force);

    typedef typename ProblemType::DiffusionFactorType::NonparametricType DiffusionFactorType;
    const auto diffusion_factor_minimizing = disc.problem().diffusion_factor()->with_mu(disc.map_parameter(mu_minimizing,
                                                                                                           "diffusion_factor"));
    typedef typename ProblemType::DiffusionTensorType::NonparametricType DiffusionTensorType;
    const auto diffusion_tensor_minimizing = disc.problem().diffusion_tensor()->with_mu(disc.map_parameter(mu_minimizing,
                                                                                                           "diffusion_tensor"));
    ...
    const  LocalOperator::Codim0Integral< LocalEvaluation::Product< ConstantFunctionType > >
      local_eta_r_product(over_integrate, constant_one);
    const auto eta_r_difference = *force - p0_force;

    std::vector< DynamicMatrix< RangeFieldType > > tmp_matrices(local_eta_r_product.numTmpObjectsRequired(),
                                                                DynamicMatrix< RangeFieldType >(1, 1, 0.0));
    DynamicMatrix< RangeFieldType > local_result_matrix(1, 1, 0.0);
  ...
  static RangeFieldType compute_diffusive_flux_estimator(const DiscretizationType& disc,
                                                         const VectorType& vector,
                                                         const ParametersType& parameters)
  {
    using namespace GDT;
    ...
    typedef typename ProblemType::DiffusionFactorType::NonparametricType DiffusionFactorType;
    const auto diffusion_factor = disc.problem().diffusion_factor()->with_mu(disc.map_parameter(mu,
                                                                                                "diffusion_factor"));
    typedef typename ProblemType::DiffusionTensorType::NonparametricType DiffusionTensorType;
    const auto diffusion_tensor = disc.problem().diffusion_tensor()->with_mu(disc.map_parameter(mu,
                                                                                                "diffusion_tensor"));
    ...
    const LocalOperator::Codim0Integral< LocalEvaluation::ESV2007::DiffusiveFluxEstimate< DiffusionFactorType
                                                                                        ...
                                                                                        , DiffusionTensorType > >
      local_eta_df_product(over_integrate, *diffusion_factor_fixed, *diffusion_tensor_fixed, diffusive_flux);

    std::vector< DynamicMatrix< RangeFieldType > > tmp_matrices(local_eta_df_product.numTmpObjectsRequired(),
                                                                DynamicMatrix< RangeFieldType >(1, 1, 0.0));
    DynamicMatrix< RangeFieldType > local_result_matrix(1, 1, 0.0);
    ...
    return std::sqrt(eta_df_squared);
  } // ... compute_diffusive_flux_estimator(...)

  static RangeFieldType compute_estimator(const DiscretizationType& disc,
                                          const VectorType& vector,
                                          const ParametersType& parameters)
  {
    using namespace GDT;
    ...
    assert(!disc.problem().diffusion_tensor()->parametric());
    assert(!disc.problem().force()->parametric());
    assert(!disc.problem().dirichlet()->parametric());
    assert(!disc.problem().neumann()->parametric());
    const double alpha = disc.problem().diffusion_factor()->alpha(mu, mu_fixed);
    const double gamma = disc.problem().diffusion_factor()->gamma(mu, mu_fixed);
    assert(alpha > 0.0);
    assert(gamma > 0.0);
    const double one_over_sqrt_alpha = 1.0/std::sqrt(alpha);
    const double sqrt_gamma = std::sqrt(gamma);
    const double sqrt_gamma_tilde = std::max(sqrt_gamma, one_over_sqrt_alpha);

    return one_over_sqrt_alpha*(sqrt_gamma*std::sqrt(eta_nc_squared)
...
public:
  ...
  {
    return {
        BaseType::nonconformity_estimator_id()
      , BaseType::residual_estimator_id()
      , BaseType::diffusive_flux_estimator_id()
      , BaseType::estimator_id()
    };
  } // ... available()
  ...
  static RangeFieldType estimate(const DiscretizationType& disc,
                                 ...
                                 const ParametersType& parameters)
  {
    BaseType::assert_parameters(disc, parameters);
    if (type == BaseType::nonconformity_estimator_id())
      return BaseType::compute_nonconformity_estimator(disc, vector, parameters);
    else if (type == BaseType::residual_estimator_id())
      return BaseType::compute_residual_estimator(disc, vector, parameters);
    else if (type == BaseType::diffusive_flux_estimator_id())
      return BaseType::compute_diffusive_flux_estimator(disc, vector, parameters);
    else if (type == BaseType::estimator_id())
      return BaseType::compute_estimator(disc, vector, parameters);
    else
      DUNE_THROW(Stuff::Exceptions::you_are_using_this_wrong,
                 "type '" << type << "' is not one of available_estimators()!");
  } // ... estimate(...)
...
public:
  ...
  typedef typename Estimator::ParametersType ParametersType;
...
//    , outside_inside_coupling_matrices_(ms_grid_->size())
  ...
  RangeFieldType estimate(const VectorType& vector,
                          ...
                          const ParametersType parameters = ParametersType({{"fixed", Pymor::Parameter()},
                                                                            {"minimizing", Pymor::Parameter()},
                                                                            {"current", Pymor::Parameter()}})) const
  {
